...
export default function Blog() {
  ...
  return (
    <div className="h-auto mb-10" id="blog">
      <h2 className="font-sans xs:text-5xl text-10xl md:text-20xl ml-8 md:ml-20 font-medium text-black-t-90">Blog</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-20 relative -top-20 md:-top-44 ml-10 md:ml-12">
        {
          blogPosts.map((post) => (
            <div key={post.slug}>
              <h4 className="xs:text-4xl text-5xl font-medium text-orange font-sans tracking-wider">{post.title}</h4>
              <p className="text-3xl font-mono text-bluegray pt-2 mb-7">{format(parseISO(post.date), 'MMMM, do uuu')}</p>
