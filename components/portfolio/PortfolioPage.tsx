...
export default function Portfolio() {
  ...
  return (
    <div className="flex flex-col">
      <h2 className="text-green text-5xl md:text-7xl lg:text-9xl mt-16 md:mt-32 mb-20 md:mb-44 font-bold">.portfolio(<span className="text-white">5</span>)</h2>
      <div className="flex flex-col">
        {
          pfData.map((project) => {
            return (
              <div key={project.id} className="flex flex-col md:flex-row mb-32 mx-5 md:mx-0">
                <div className="w-full md:w-1/2">
                  <img src={project.image} alt={project.title} />
                </div>
                <div className="md:ml-20 md:w-1/2">
                  <h2 className="text-4xl lg:text-5xl font-bold text-orange mb-5 mt-10 md:mt-0">{project.title}</h2>
                  <p className="text-2xl lg:text-3xl text-green mb-5">{project.languages}</p>
                  <p className="text-md md:text-xl text-white mb-16 leading-loose">{project.blurb}</p>
